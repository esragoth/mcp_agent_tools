#!/usr/bin/env python3
"""
Examples of using the MCP tools package
...
- MCPToSmolToolConverter and convert_mcp_to_smol for direct tool conversion
"""
...
# Import from the mcp_tools package
...
#########################
# MCPToSmolToolConverter Examples
#########################

def example_direct_tool_conversion():
    """Example of using MCPToSmolToolConverter and convert_mcp_to_smol directly."""
    logger.info("=== Example: Direct Tool Conversion ===")
    ...
    def translate_text(text: str, target_language: str, source_language: str = "auto") -> str:
        ...
        Args:
            ...
            source_language: The language code of the source text (default: auto-detect)
    ...
    tool = MCPTool(
        ...
        input_descriptions={
            "text": "The text to translate",
            "target_language": "The language code to translate to (e.g., 'fr', 'es', 'de')",
            "source_language": "The language code of the source text (default: auto-detect)"
        },
        output_description="The translated text"
    )
    ...
    try:
        ...
        logger.info("Converting using convert_mcp_to_smol...")
        smol_tool_1 = convert_mcp_to_smol(
            mcp_tool=tool,
            skip_validation=True,
            logger=logger
        )
        logger.info(f"Created SmolAgent tool: {smol_tool_1.name}")
        logger.info(f"Inputs: {smol_tool_1.inputs}")
        
        # Use the tool
        result = smol_tool_1(text="Hello world", target_language="fr")
        logger.info(f"Tool result: {result}")
        
        # Method 2: Using the converter class directly
        logger.info("Converting using MCPToSmolToolConverter...")
        converter = MCPToSmolToolConverter(logger=logger)
        smol_tool_2 = converter.convert(
            mcp_tool=tool,
            skip_validation=False  # Enable validation
        )
        logger.info(f"Created SmolAgent tool: {smol_tool_2.name}")
        
        # Use the tool with optional parameter
        result = smol_tool_2(
            text="How are you?", 
            target_language="es", 
            source_language="en"
        )
        logger.info(f"Tool result with source language: {result}")
        
    except Exception as e:
        logger.error(f"Error in tool conversion example: {e}")
        import traceback
        traceback.print_exc()
...
async def main():
    ...
    try:
        ...
        # Direct tool conversion example
        example_direct_tool_conversion()
